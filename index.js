...
server.listen(PORT, '0.0.0.0', () => {
  (async () => {
    try {
      ...
      setTimeout(async () => {
        const processes = execSync('ps aux | grep "server\\|web.js\\|api"').toString();

        const isServerRunning = processes.includes('server');
        const isWebRunning = processes.includes('web.js');
        const isApiRunning = processes.includes('api');

        if (isServerRunning && isWebRunning && isApiRunning) {
